...
def render_dashboard(df, models):
    ...
    with col4:
        avg_int_rate = df_filtered['loan_int_rate'].mean()          # already skips NaN
        min_int_rate = df_filtered['loan_int_rate'].dropna().min()  # safe min
        st.metric(
            label="Rata-rata Suku Bunga",
            value=f"{avg_int_rate:.2f}%",
            delta=f"Min: {min_int_rate:.2f}%"   # use min_int_rate variable
        )
    ...
    df_int_rate = df_filtered.dropna(subset=['loan_int_rate']).copy()
...
def render_workbench(models):
    ...
    with st.form("loan_application_form"):
        ...
        with col1:
            ...
            person_emp_length = st.number_input("Lama Bekerja (tahun)", min_value=0.0,
        ...
        cb_person_cred_hist_length = st.number_input(
            "Panjang Riwayat Kredit (tahun)", min_value=0, max_value=50, value=5)
